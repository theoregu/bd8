...
if($result = $mysqli->query('SELECT * FROM workers')) {   // $mysqli - наш об'єкт, через який здійснюємо підключення, query - метод, який дозволяє виконати довільний запит
    ...
    printf("<table><tr><th>ІД Працівника</th><th>Ім'я Працівника</th><th>Прізвище Працівника</th><th>Посада Працівника</th></tr>");   // <br> в html - розрив рядка
    ...
    while( $row = $result->fetch_assoc() ){ // fetch_assoc() повертає рядок із запиту у вигляді асоціативного масиву, наприклад $row = ['id'=>'1', 'pib'=>'Олександр', 'grupa'=>'ІПЗ-31']
        printf("<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>", $row['id'], $row['Name'], $row['SecondName'], $row['Position']); //виводимо результат на сторінку
    };
    printf("</table>");
    /*Звільняємо пам'ять*/
    $result->close();
...
<ul>
    <li><a href="insertWorkersForm.php">Вставити рядок</a><br></li>
